...
export default function CareerIdentity() {
  return (
    <div className="mx-auto max-w-6xl px-4 py-8 sm:px-6 lg:py-12 space-y-12 animate-in fade-in slide-in-from-bottom-4 duration-700">
      <div>
        <h2 className="mb-3 font-display text-4xl font-bold tracking-tight">Your Career Identity</h2>
        <p className="max-w-6xl text-lg text-muted-foreground">
          Based on your experience, competencies, work patterns, and AI readiness, CareerShift has created a personalized career identity that reflects where you are today and where you can grow next.
        </p>
      </div>
        ...
        <div className="relative z-10">
          ...
          <div className="flex flex-wrap gap-2 mb-8">
            {["Backend", "Healthcare", "Leadership", "Cloud", "AI Ready"].map(badge => (
              <span key={badge} className="rounded-md bg-background/80 px-3 py-1.5 text-sm font-semibold text-foreground shadow-sm ring-1 ring-border">
                {badge}
              </span>
            ))}
          </div>
          ...
          {[
            ...
            <div key={node.label} className="flex flex-col md:flex-row items-center gap-4 w-full md:w-auto">
              <div className="flex flex-col items-center text-center">
                <span className="text-xs font-bold uppercase tracking-wider text-muted-foreground mb-2">{node.label}</span>
                <span className={`font-semibold ${i === arr.length - 1 ? 'text-primary text-lg' : 'text-foreground'}`}>{node.role}</span>
              </div>
              {i < arr.length - 1 && (
      ...
      <div>
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {[
            { role: "AI Backend Engineer", reason: "Leverages your deep backend skills while integrating AI services natively into applications." },
            { role: "Platform Engineer", reason: "Fits your strong architectural thinking and desire to improve developer productivity." },
            { role: "Solutions Architect", reason: "A natural progression for your system design strengths and emerging leadership." },
            { role: "Engineering Manager", reason: "Capitalizes on your solid technical foundation to guide and mentor technical teams." },
            { role: "AI Solutions Consultant", reason: "Combines your domain expertise (Healthcare) with cutting-edge AI implementation." },
          ].map(role => (
            <div key={role.role} className="flex flex-col rounded-xl border border-border bg-background p-6 shadow-sm hover:shadow-md transition-shadow">
      ...
      <div className="rounded-2xl border border-primary bg-primary/5 p-8 shadow-soft">
        ...
        <p className="text-lg leading-relaxed text-foreground/90 font-medium">
          Continue strengthening your technical expertise while expanding into AI-assisted software engineering. Focus on leadership, system thinking, and workflow automation to future-proof your career.
        </p>
      </div>

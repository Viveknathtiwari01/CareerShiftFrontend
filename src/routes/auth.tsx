...
import { ArrowRight, BrainCircuit, Loader2, Lock, Mail, User as UserIcon } from "lucide-react";
...
export const Route = createFileRoute("/auth")({
  head: () => ({
    meta: [
      { title: "Sign in · CareerShift" },
      {
        name: "description",
        content:
          "Sign in or create your CareerShift account to run your AI Career Readiness assessment.",
      },
    ],
  }),
  component: AuthPage,
...
function AuthPage() {
  ...
  useEffect(() => {
    if (!loading && user) {
      navigate({ to: user.onboarded ? "/dashboard" : "/onboarding", replace: true });
    }
      ...
      <aside className="bg-primary relative hidden overflow-hidden text-primary-foreground lg:flex lg:flex-col lg:justify-between lg:p-12">
        <Link to="/" className="flex items-center gap-2">
          <div className="relative grid h-9 w-9 place-items-center rounded-lg bg-white/10 ring-1 ring-white/20">
            ...
            <span className="absolute -right-0.5 -top-0.5 h-2 w-2 rounded-full bg-warm ring-2 ring-navy" />
          </div>

      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
        {progressSteps.map((step, idx) => (
          <div key={idx} className="flex flex-col items-center p-3 text-center bg-card rounded-lg border border-border shadow-sm">
            ...
            <p className="text-xs font-semibold text-foreground mb-1 leading-tight">{step.name}</p>
            <p className={`text-[10px] font-medium uppercase tracking-wider ${step.status === 'Next' || step.status === 'Ready' ? 'text-primary' : 'text-muted-foreground'}`}>
              {step.status}
            </p>
          </div>
        ))}
